...
'''
def executeORAWbatch_roi(myWorkingDirectory,roi,exportDir,export_format,export_name):
  outPath = r''
...
#      logger.info('processing patient: %s', patient[entry])
          ...
          try:
              ...
              if export_format == 'csv': # sava results in csv
                  try:
                      ...
                      Mask= []
                      result=[]
                      frames = pandas.concat([frames,results],axis=1)
                  except Exception:
                    logger.error('FEATURE EXTRACTION FAILED for CSV output:', exc_info=True)
              else:# save results in triple stroe
                  try:
                    featureVector = PyrexWithParams.CalculationRun(Image,Mask,paramsFile) #compute radiomics
                    featureVector.update({'patient':patient[entry],'contour':roi}) #add patient ID and contour                
                    PyrexOutput.RadiomicsRDF(featureVector,exportDir,patient[entry],roi,export_format,export_name) #store radiomics locally with a specific format 
                    logger.info('Extraction complete, writing rdf')
                  except Exception:
                    logger.error('FEATURE EXTRACTION FAILED for RDF output:', exc_info=True)
          except Exception:
  ...
  #fullresults = pandas.concat(frame)
#      outputFilepath = os.path.join(exportDir,'testpatient'+'.csv')      
  frames.T.to_csv(outputFilepath, index=False, na_rep='NaN')
  logger.info('CSV writing complete')
  logger.info('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%')
'''
